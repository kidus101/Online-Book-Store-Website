...
const OrderTable = () => {
  ...
  const handleCancelOrder = async (bookId, orders) => {
    try {
      ...
      //   const updatedBooksResponse = orderedBooksResponse.filter(book => book.id !== bookId);
      //   setOrderedBooksResponse(updatedBooksResponse);

      //   // Set the orderId state after deleting the order
      //   setOrderId(order.orderId);

      //   return order.orderId; // Return the orderId if the bookId matches
      // }
    } catch (error) {
